...
const closeTaskCompleted = async(task) =>{
  ...
  const getTime = async (task_id) =>{
    console.log(task_id)
    var timedata = await axios.post(`${server}/api/admin/task_time`, { task_id: task_id });
  ...
  return (
    ...
    <div className="buttonalign">
      ...
      <GridItem>{/* Filter project form Start */}
            ...
            <Typography>
              {TimeData.map((time)=>{
                return(
                  <>
                    <p>{time.username}</p>
                    <p>Estimate Time-{time.estimate_time} , Spent Time-{time.spent_time}</p>
                  </>
                )
              })}
            </Typography>
